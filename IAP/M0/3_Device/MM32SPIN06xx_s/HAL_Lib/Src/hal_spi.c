...
u32 SPI_ReceiveData(SPI_TypeDef* spi)
{
    ...
    u32 temp = READ_REG(spi->RDR);

    u8 templen = READ_REG(spi->ECR);
    if(templen  == 0)
        templen = 32;
...
bool SPI_DataSizeConfig(SPI_TypeDef* spi, u8 data_size)
{
    if (data_size > 32)
        return false;
    CLEAR_BIT(spi->GCR, SPI_DataSize_32b & 0x1F);
    SET_BIT(spi->GCR, (u32)data_size);
    return true;
}

//////////////////////////////////////////////////////////////////////////////////
// void SPI_DataSizeConfig(SPI_TypeDef* spi, SPI_DataSize_TypeDef SPI_DataSize)
//{
