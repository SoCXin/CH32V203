...
void I2C_Init(I2C_TypeDef* I2Cn, I2C_InitTypeDef* pInitStruct)
{
    ...
    I2Cn->IC_ENABLE &= ~I2C_ENR_ENABLE;
    RCC_GetClocksFreq(&rcc_clocks);

    pclk1       = rcc_clocks.PCLK1_Frequency;
    pclk1Period = 1000000000 / pclk1;
    i2cPeriod   = 1000000000 / pInitStruct->I2C_ClockSpeed;

    minSclLowTime = pclk1 / pInitStruct->I2C_ClockSpeed ;
    i2cPeriod = 82 / pclk1Period;

    if (pInitStruct->I2C_ClockSpeed <= 100000) {
        I2Cn->IC_SS_SCL_LCNT = (minSclLowTime - 13 - i2cPeriod) / 2;
        I2Cn->IC_SS_SCL_HCNT = (minSclLowTime - 13 - i2cPeriod - I2Cn->IC_SS_SCL_LCNT);
    }
    else {
        I2Cn->IC_FS_SCL_LCNT = (minSclLowTime - 13 - i2cPeriod ) / 2 + 4;
        I2Cn->IC_FS_SCL_HCNT = (minSclLowTime - 13 - I2Cn->IC_FS_SCL_LCNT - i2cPeriod);
    }

    I2Cn->IC_CON &= ~(I2C_CR_EMPINT     |   \
                      I2C_CR_SLAVEDIS   |   \
                      I2C_CR_REPEN      |   \
                      I2C_CR_MASTER10   |   \
                      I2C_CR_SLAVE10    |   \
    ...
    I2Cn->IC_CON =  I2C_CR_EMPINT       |   \
                    I2C_CR_REPEN        |   \
                    ((pInitStruct->I2C_Speed) ? I2C_CR_FAST : I2C_CR_STD) | \
                    ((pInitStruct->I2C_Mode)  ? I2C_CR_MASTER : 0x00);
    I2Cn->IC_INTR_MASK &= INTR_MASK;
...
{
    pInitStruct->I2C_Mode       = I2C_CR_MASTER;
    pInitStruct->I2C_OwnAddress = I2C_OWN_ADDRESS;
    pInitStruct->I2C_Speed      = I2C_CR_STD;
    pInitStruct->I2C_ClockSpeed = 100000;
}
...
{
    (state) ? (I2Cn->IC_ENABLE |= I2C_ENR_ENABLE) : (I2Cn->IC_ENABLE &= ~I2C_ENR_ENABLE);
}
...
void I2C_DMACmd(I2C_TypeDef* I2Cn, FunctionalState state)
{
    if (state) {
        if (I2C_DMA_DIR == TDMAE_SET)
            I2Cn->IC_DMA_CR |= TDMAE_SET;

        else
            I2Cn->IC_DMA_CR |= RDMAE_SET;
    }
    else
        I2Cn->IC_DMA_CR &= ~(I2C_DMA_RXEN | I2C_DMA_TXEN);
}
...
{
    (state) ? (I2Cn->IC_CON |= I2C_CR_REPEN) : (I2Cn->IC_CON &= ~I2C_CR_REPEN);
}
...
void I2C_GenerateSTOP(I2C_TypeDef* I2Cn, FunctionalState state)
{
    ...
    I2Cn->IC_ENABLE |= I2C_ENR_ABORT;

    while (I2Cn->IC_ENABLE & I2C_ENR_ABORT) {
        if (overTime-- == 0)
            break;
    }
    I2Cn->IC_CLR_TX_ABRT;
...
{
    MODIFY_REG(I2Cn->IC_TAR, (u16)I2C_TAR_ADDR, (u16)(addr >> 1));
}
...
{
    (state) ? (I2Cn->IC_TAR |= IC_TAR_ENDUAL_Set) : (I2Cn->IC_TAR &= IC_TAR_ENDUAL_Reset);
}
...
{
    (state) ? (I2Cn->IC_TAR |= I2C_TAR_SPECIAL) : (I2Cn->IC_TAR &= ~I2C_TAR_SPECIAL);
}
...
void I2C_ITConfig(I2C_TypeDef* I2Cn, u16 it, FunctionalState state)
{
    if (it == I2C_IT_RX_FULL)
        I2C_ReadCmd(I2Cn);
    (state) ? SET_BIT(I2Cn->IC_INTR_MASK, it) : CLEAR_BIT(I2Cn->IC_INTR_MASK, (u16)it);
}
...
{
    return (u32)I2Cn->IC_RAW_INTR_STAT & FLAG_Mask;
}
...
ErrorStatus I2C_CheckEvent(I2C_TypeDef* I2Cn, u32 event)
{
    ...
    return (ErrorStatus)((I2Cn->IC_RAW_INTR_STAT & event) == event);
}
...
{
    if (flag & 0x8000)
        return ((I2Cn->IC_STATUS & flag) ? SET : RESET);

    if ((flag == I2C_FLAG_RX_FULL) && (I2C_CMD_DIR == 0)) {
        I2Cn->IC_DATA_CMD = I2C_DR_CMD;
        I2C_CMD_DIR       = 1;
    }
    return (((I2Cn->IC_RAW_INTR_STAT & flag)) ? SET : RESET);
}
...
{
    (state) ? CLEAR_BIT(I2Cn->IC_CON, I2C_CR_SLAVEDIS) : SET_BIT(I2Cn->IC_CON, I2C_CR_SLAVEDIS);
}
...
{
    WRITE_REG(I2Cn->IC_DMA_CR, (2 - dir));
}
