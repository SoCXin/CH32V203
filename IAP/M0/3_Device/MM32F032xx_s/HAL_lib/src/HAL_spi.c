...
void SPI_SendData(SPI_TypeDef* SPIx, u32 data)
{
    ...
    templen = READ_REG(SPIx->ECR);
    if(templen  == 0) templen = 32;
...
void SPI_BiDirectionalLineConfig(SPI_TypeDef* SPIx, SPI_Direction_TypeDef direction)
{
    switch (direction) {
        case SPI_Direction_Rx:
            SET_BIT(SPIx->GCR, SPI_GCR_RXEN);
            break;
        case SPI_Direction_Tx:
            SET_BIT(SPIx->GCR, SPI_GCR_TXEN);
            break;
        case SPI_Disable_Rx:
            CLEAR_BIT(SPIx->GCR, SPI_GCR_RXEN);
            break;
        case SPI_Disable_Tx:
            CLEAR_BIT(SPIx->GCR, SPI_GCR_TXEN);
            break;
        default:
            break;
    }
