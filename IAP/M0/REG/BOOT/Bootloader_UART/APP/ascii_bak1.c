...
static QUEUE8_t QueueHexRx         = {0};
static QUEUE8_t QueueBinaryRx      = {0};
static uint8_t  HexRxBuf[50]     = {0};  //2048
static uint32_t HexRxlen;
static uint32_t BinaryRxlen;
static uint8_t  BinaryRxBuf[1500]  = {0};
...
static uint8_t NeedParseHexLine = 0 ;
...
/******************************************************************************
 * @brief  Read data from Hex data buffer
 * @param  buffter , buffterSize
 * @retval The result of reading
 * @attention  None
******************************************************************************/
static uint32_t HEX_RxRead(uint8_t *Buffter, uint32_t BuffterSize)
{
    return Queue_PacketOut(&QueueHexRx, Buffter, BuffterSize);
}

/******************************************************************************
 * @brief  Write data to Hex data buffer
 * @param  buffter , writeLen
 * @retval The result of writing
 * @attention  None
******************************************************************************/
static uint32_t HEX_RxWrite(uint8_t *Buffter, uint32_t WriteLen)
{
    return Queue_PacketIn(&QueueHexRx, Buffter, WriteLen);
}

/******************************************************************************
 * @brief  Read data from Binary data buffer
 * @param  buffter , buffterSize
 * @retval The result of reading
 * @attention  None
******************************************************************************/
static uint32_t Binary_RxRead(uint8_t *Buffter, uint32_t BuffterSize)
{
    return Queue_PacketOut(&QueueBinaryRx, Buffter, BuffterSize);
}

/******************************************************************************
 * @brief  Write data to Binary data buffer
 * @param  buffter , writeLen
 * @retval The result of writing
 * @attention  None
******************************************************************************/
static uint32_t Binary_RxWrite(uint8_t *Buffter, uint32_t WriteLen)
{
    return Queue_PacketIn(&QueueBinaryRx, Buffter, WriteLen);
}
...
{
    uint8_t ResByte;
    static uint8_t HexRxIndexTemp = 0 ;
...
//        UartRxCnt++;
        ...
        else if(ResByte == 0x0A)  /* LF */
        {
            if(HexRxBuf[HexRxIndexTemp-1] == 0x0D)  /* CR */
            {
                HexRxBuf[HexRxIndexTemp]=ResByte ;
                HexRxIndexTemp = 0 ;
                NeedParseHexLine = 1;
            }
        }
...
{
    static uint32_t RowNum = 0 ;
    static uint32_t NextOffsetAddr = 0 ;
    static int8_t TempResult = 1;
    uint8_t *pTempBinData = 0x00;
    uint8_t TempBinDataBuf[16] = {0};
    volatile uint8_t    ErrorCode = 0;
    t_HexLineInfo   HexDataInfo;
    /* Judge whether all data has been parsed and stored in the cache flag */
    uint8_t CanGetData = TRUE;
    ...
    /* Create HEX receive queue */
    Queue_PacketCreate(&QueueHexRx, HexRxBuf, sizeof(HexRxBuf));
    /* Create Bin receive queue */
    Queue_PacketCreate(&QueueBinaryRx, BinaryRxBuf, sizeof(BinaryRxBuf));
    ...
    while(1)
	{
        if(NeedParseHexLine)
        {
            NeedParseHexLine = 0;
            /* Get hex data information */
            ErrorCode = GetHexLineInfo(HexRxBuf, &HexDataInfo);
            if (ErrorCode == RIGHT)
            {
                if (HexDataInfo.Rectype == RCD_DATA)
                {
              /* Put the hex data out and store it in the hexdatatemp data buffer */
                    ErrorCode = GetHexData(&HexDataInfo, TempBinDataBuf);
                    Binary_RxWrite(TempBinDataBuf, HexDataInfo.DataLength);
//                    Iap_WriteAppBin((APPLICATION_ADDRESS + NextOffsetAddr),   \
//                       ProgramFlashDataTemp,  HexDataInfo.DataLength) ;
//                    NextOffsetAddr += HexDataInfo.DataLength;
                }
            }
        }
        /* Get queue data length */
        BinaryRxlen = Queue_PacketLengthGet(&QueueBinaryRx);
        if(BinaryRxlen>255)
        {
            Binary_RxRead(ProgramFlashDataTemp, 256);
            Iap_WriteAppBin((APPLICATION_ADDRESS + NextOffsetAddr*256),   \
                       ProgramFlashDataTemp,  256) ;
            NextOffsetAddr += 1;
        }
